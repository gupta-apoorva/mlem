...
#ifndef _CSR4MATRIX_HPP_
#define _CSR4MATRIX_HPP_
...
public:
    ...
    node_iterator beginRow(uint32_t rowNr, const std::vector<uint64_t>& rowidx) const {
        assert(rowidx[rowNr] < nnz);
        return node_iterator(&data[rowidx[rowNr] -
            this->mappings[this->currentMap].mapFirstElement]);
    }
    node_iterator endRow(uint32_t rowNr, const std::vector<uint64_t>& rowidx) const {
        assert(rowidx[rowNr + 1] <= nnz);
        return node_iterator(&data[rowidx[rowNr + 1] -
            this->mappings[this->currentMap].mapFirstElement]);
    }
