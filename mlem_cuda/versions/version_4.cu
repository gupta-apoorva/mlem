          ...
          Vector<float>& image, int nIterations, int checkpointing)
{
    ...
    d_tran_mat_vec_mul <<<grid, block>>> (norm_d, rowVector_d , csrVal_d, csrRowInd_d, csrColInd_d, num_rows_this_rank);
    ...
    for (int iter = iter_alt; iter<nIterations ; iter++){                                                                                                                                                                                                                                                                                                                                                       
        ...
        start_time_measurement(start);
        d_mat_vec_mul <<<grid , block>>> (  &fwproj_d[myrange.start], 
                                image_d, 
                                csrVal_d, 
                                csrRowInd_d, 
                                csrColInd_d, 
        ...
        start_time_measurement(start);
        d_tran_mat_vec_mul <<<grid, block>>>(update_d, &correlation_d[myrange.start], csrVal_d, csrRowInd_d, csrColInd_d, num_rows_this_rank);
