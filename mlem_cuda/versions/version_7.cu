          ...
          Vector<float>& image, int nIterations, int checkpointing)
{
    ...
    if (mpi.rank == 0)
        for (int i = 0; i< parts_to_further_break_matrix ; i++)
            std::cout << splitting_based_on_rows[i].start << " " << splitting_based_on_rows[i].end << std::endl;
    ...
    for (int iter = iter_alt; iter<nIterations ; iter++)
    {
        ...
        // Calculating the forward projection
        start_time_measurement(start);
        for (int i=parts_to_further_break_matrix-1 ; i>=0 ; i--)
        {
            ...
            mat_vec_mul_warp <<<grid, block, block.x*sizeof(float)>>> (x, z ,&csrRowInd_d[splitting_based_on_rows[i].start] ,csrColInd_d ,csrVal_d ,image_d ,&fwproj_d[myrange.start + splitting_based_on_rows[i].start]);
        }
        stop_time_measurement(start, stop, &time_runtime.timing_loop[iter].fwproj_time);

        // Reducing the forward projection
        start_time_measurement(start);
