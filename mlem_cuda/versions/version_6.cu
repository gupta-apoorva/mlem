...
// #endif
    ...
    for (int iter = iter_alt; iter<nIterations ; iter++){                                                                                                                                                                                                                                                                                                                                                       
        ...
        start_time_measurement(start);
        mat_vec_mul_warp <<<grid, block, block.x*sizeof(float)>>> (0, num_rows_this_rank, csrRowInd_d , csrColInd_d , csrVal_d , image_d ,&fwproj_d[myrange.start]);
    ...
    // Freeing rowVector_d whenever it is allocated
    #ifdef CUSPARSE
        cudaFree(rowVector_d);
    #endif
