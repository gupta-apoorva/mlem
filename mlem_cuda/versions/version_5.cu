          ...
          Vector<float>& image, int nIterations, int checkpointing)
{
    ...
    trans_mat_unit_vec_mul_warp <<<grid, block>>> (0, num_rows_this_rank, &csrRowInd_d[0], csrColInd_d, csrVal_d, norm_d);
    stop_time_measurement(start, stop, &time_runtime.norm_calc_time);

    // Reducing the norm
    start_time_measurement(start);
    MPI_Allreduce(MPI_IN_PLACE, norm_d, nColumns, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
    ...
    for (int iter = iter_alt; iter<nIterations ; iter++){                                                                                                                                                                                                                                                                                                                                                       
        ...
        start_time_measurement(start);
        mat_vec_mul_warp <<<grid, block, block.x*sizeof(float)>>>  (  0, 
                                                                        num_rows_this_rank , 
                                                                        csrRowInd_d , 
                                                                        csrColInd_d , 
                                                                        csrVal_d , 
                                                                        image_d , 
                                                                        &fwproj_d[myrange.start]);
        stop_time_measurement(start, stop, &time_runtime.timing_loop[iter].fwproj_time);

        // reducing Forward Projection
        start_time_measurement(start);
        MPI_Allreduce(MPI_IN_PLACE, fwproj_d, nRows, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
        ...
        start_time_measurement(start);
        trans_mat_vec_mul_warp <<<grid, block>>> (0, num_rows_this_rank, csrRowInd_d, csrColInd_d, csrVal_d, &correlation_d[myrange.start], update_d);
        stop_time_measurement(start, stop, &time_runtime.timing_loop[iter].update_calc_time);

        // Reducing backward projection
        start_time_measurement(start);
        MPI_Allreduce(MPI_IN_PLACE, update_d, nColumns, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
