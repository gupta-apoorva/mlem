...
{
    laik_switchto_new_phase(norm, world, laik_All, 
    ...
    laik_switchto_new_phase(norm, world, laik_All, LAIK_DF_CopyIn);
...
{
    laik_switchto_new_phase(result, world, laik_All,
                      LAIK_DF_Init | LAIK_DF_ReduceOut | LAIK_DF_Sum);
                ...
                std::chrono::duration<float>& total_time)
{
    ...
    laik_switchto_new_phase(update, world,  laik_All, LAIK_DF_CopyIn);
...
void mlem(Laik_Instance* inst, Laik_Group* world, Laik_AccessPhase* ap,
          ...
          Vector<float>& image, int nIterations)
{
    ...
    calcColumnSums(world, ap, matrix, norm);
    ...
    for (int iter=0; iter<nIterations; ++iter) {
        ...
        calcFwProj(world, ap, matrix, image, fwproj, compute_time, total_time);
        calcCorrel(fwproj, lmsino, correlation, compute_time, total_time);
        calcBkProj(world, ap, matrix, correlation, update, compute_time, total_time);
...
int main(int argc, char *argv[])
{
    ...
    mlem(inst, world, ap, matrix, lmsino, image, progops.iterations);
